...
class issue(models.Model):
    ...
    def _values(self, prepaid_hours_id):
        used_time = 0
        tobeapprove_time = 0
        approval_lines_obj = self.env['account.analytic.prepaid_hours_approval_line'].search([('prepaid_hours_id','=',prepaid_hours_id)])
        for approval_line in approval_lines_obj:
            if approval_line.approval_id.state == 'approved':
                ...
                if approval_line.approval_id.state == '2b_approved':
                    tobeapprove_time = tobeapprove_time + approval_line.requested_hours
    ...
    def _calculate_extra_amount(self):
        ...
        price = self.project_id.analytic_account_id.invoice_type_id.search([('name','=', )]).product_price

    def _create_approval_values(self, approval_id,prepaid_hours_id):
        approval_values_obj = self.env['account.analytic.prepaid_hours_approved_values']
        ...
        approval_values_values = {
                                ...
                                'extra_hours': values['remaining_time'] - self.feature_id.expected_hours,
                                #'extra_amount':,
                                'approval_id': approval_id,
                                    }

    def _create_approval_line(self, approval_id):
        feature_obj = self.env['project.scrum.feature'].browse(self.feature_id.id)
        approval_line_obj = self.env['account.analytic.prepaid_hours_approval_line']
        for hour_type in self.feature_obj.hour_ids:
            prepaid_hours_id = self.project_id.analytic_account_id.acc_analytic_qty_grp_ids.search([('name','=',hour_type.name)])[0].acc_analytic_qty_grp_id.id
