...
class PayWizard(osv.TransientModel):
    ...
    def _get_interval_period(
        ...
        cr, uid, [('fiscalyear_id', '=', fiscal_year),
            ('date_stop','<=',end_period.date_stop),('special','=',False)])
        return period_list

    def _invoices_without_commission(self, cr, uid, wizard, member,\
                                      rule, period_ids,context=None):
                ...
                if payment.journal_id and payment.journal_id.pay_commission:
                    ...
                    if total_credit > 0.0:
                        if invoice.force_commission:
                            ...
                            commission_id = \
                             commission_obj.create(cr, uid, values, \
                                                    context=context)
                            ...
                            for rule_line in rule_lines:
                                ...
                                if rule_line.partner_category_id:
                                    if not rule_line.partner_category_id \
                                     in invoice.partner_id.category_id:
                                ...
                                if result:
                                    if rule_line.commission_percentage and \
                                        total_credit:
                                        amount = total_credit * \
                                        rule_line.commission_percentage / 100
                                    else:
                                        amount = 0.0
                                    exp_days = relativedelta(
    ...
    def _invoices_with_commission(
        self, cr, uid, wizard, member, rule, period_ids, context=None):
        invoice_obj =self.pool.get('account.invoice')
        rule_line_obj = self.pool.get('sale.commission.rule.line')
        commission_obj = self.pool.get('sale.commission.commission')
        commission_ids = []
    ...
    def do_payment(self, cr, uid, ids, context=None):
        ...
        period_ids = self._get_interval_period(
                        cr, uid, wizard.period_id, context=context)
        # Get the journals that pay commissions
        journal_obj = self.pool.get('account.journal')
        journal_ids = journal_obj.search(
                    cr, uid, [('pay_commission','=',True)], context=context)
    ...
    _columns = {
        ...
        'rule_ids': fields.many2many('sale.commission.rule', \
                    rel='sale_commission_wizard_rule_rel', string='Rules'),
    }
